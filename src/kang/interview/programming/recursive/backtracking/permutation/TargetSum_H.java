...
public class TargetSum_H {
	...
	/**
	 * Top-down (memorization) dynamic programming
	 */
	...
	public int findTargetSumWays_(int[] nums, int S) {
		...
		map = new HashMap<>();
		return find_(nums, 0, S);
	}

	private int find_(int[] nums, int index, int sum) {
		String code = index + ":" + sum;
		...
		int add = find(nums, index + 1, sum - nums[index]);
		int minus = find(nums, index + 1, sum + nums[index]);
		map.put(code, add + minus);
		return add + minus;
	}
