package kang.interview.programming.trie;

/**
 * 
 * With my solution I took the simple approach of giving each TrieNode a 26
 * element array of each possible child node it may have. I only gave 26
 * children nodes because we are only working with lowercase 'a' - 'z'. If you
 * are uncertain why I made the root of my Trie an empty character this is a
 ...
 * child. After evaluating the entire String the Node we left off on is marked
 * as a word this allows our Trie to know which words exist in our "dictionary"
 * 
 * For search I simply navigate through the Trie if I discover the current
 * character isn't in the Trie I return false. After checking each Char in the
 * String I check to see if the Node I left off on was marked as a word
 * returning the result.
 * 
 * Starts with is identical to search except it doesn't matter if the Node I
 * left off was marked as a word or not if entire string evaluated i always
 * return true;
 * 
 * @author yankang
 *
 */
public class Trie_iterative {
